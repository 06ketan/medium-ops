...
Resolution order (first hit wins):
  1. MEDIUM_OPS_LLM_CMD env var (full command template, "{prompt}" placeholder
     optional — prompt is otherwise piped on stdin).
  2. `claude` on PATH       -> claude --print --append-system-prompt "<sys>"
  3. `cursor-agent` on PATH -> cursor-agent --print
  4. `codex` on PATH        -> codex exec --quiet
...
def _detect() -> _Recipe:
    custom = os.environ.get("MEDIUM_OPS_LLM_CMD")
    if custom:
    ...
    raise SubprocessLLMNotFound(
        ...
        "cursor-agent, codex; or set MEDIUM_OPS_LLM_CMD='your-cli {prompt}'."
    )
...
def draft(
    *,
    ...
    recipe = _detect()
    prompt = (
        f"Story title: {post_title or '(unknown)'}\n"
        f"Response by {comment_author}:\n{comment_body.strip()}\n\n"
